...
export const handlers = [
	...
	/* profile: require Authorization header, return the logged-in user's data */
	http.get('/api/customers/{customerId}/profile', ({ params, request }) => {
		...
		// Optional: validate params.customerId matches user.customerId
		return HttpResponse.json({
			...user,
	...
	http.get(
		'/api/customers/{customerId}/transactions?limit={limit}&offset={offset}&category={category}&startDate={startDate}&endDate={endDate}&sortBy={sortBy}',
		({ request }) => {
			const auth = request.headers.get('authorization') ?? '';
			const token = auth.replace('Bearer ', '');
			const username = sessions.get(token);
